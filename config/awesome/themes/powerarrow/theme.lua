...
theme.volume = lain.widget.pulse {
    settings = function()
        vlevel = (volume_now.left + volume_now.right) / 2
        if volume_now.muted == "yes" then
            volicon:set_image(theme.widget_vol_mute)
        else
            volicon:set_image(theme.widget_vol)
        end
        volbar:set_value(vlevel / 100)
    end
}
...
local mem = lain.widget.mem({
    settings = function()
        memgraph:add_value(mem_now.perc)
    end
})
...
local cpu = lain.widget.cpu({
    settings = function()
        cpugraph:add_value(cpu_now.usage)
    end
})
...
local function numfmt(num)
    return io.popen(string.format("/usr/bin/numfmt --to=iec %d", math.floor(num))):read("*a")
end
...
function theme.at_screen_connect(s)
    ...
    s.mywibox:setup {
        ...
        { -- Right widgets
            ...
            wibox.container.background(wibox.container.margin(wibox.widget { volicon, volbar, layout = wibox.layout.align.horizontal }, 4, 7), theme.bg_focus),
            arrow(theme.bg_focus, "#777E76"),
            wibox.container.background(wibox.container.margin(wibox.widget { memicon, memgraph, layout = wibox.layout.align.horizontal }, 2, 3, 1, 1), "#777E76"),
            arrow("#777E76", "#4B696D"),
            wibox.container.background(wibox.container.margin(wibox.widget { cpuicon, cpugraph, layout = wibox.layout.align.horizontal }, 3, 4, 1, 1), "#4B696D"),
            arrow("#4B696D", "#4B3B51"),
